...
@pytest.mark.asyncio
async def test_create_list_delete_attendee(client: AsyncClient):
    """Tests full attendee lifecycle: create → list → delete → verify removal"""
    ...
    att_id = UUID(create.json()["attendee_id"])
    ...
    assert any(i["attendee_id"] == str(att_id) for i in items)
    ...
    assert all(i["attendee_id"] != str(att_id) for i in lst2.json()["items"])
