"use client";
import { getLoginCookie, setLoginCookie } from "@/actions/auth";
...
export default function LoginButton() {
  ...
  const isEmailValid =
    trimmedEmail.length > 0 && EMAIL_REGEX.test(normalizedEmail);
  ...
  const handleLogin = async () => {
    ...
    setIsLoading(true);
    try {
      await setLoginCookie(normalizedEmail);
          ...
          <div className="group z-50 relative flex w-full max-w-md flex-col gap-6 overflow-hidden rounded-3xl border border-neutral-200/70 bg-white/95 p-8 text-neutral-900 shadow-2xl shadow-amber-500/10 backdrop-blur dark:border-white/10 dark:bg-neutral-900/70 dark:text-neutral-100">
            ...
            >
              <button
                type="button"
                disabled
                className="relative flex items-center justify-center gap-2 overflow-hidden rounded-full bg-neutral-200 px-6 py-3 text-sm font-semibold text-neutral-500 opacity-80 transition dark:bg-neutral-800 dark:text-neutral-400"
              >
                <FaGoogle />
              ...
              <button
                type="submit"
                className="rounded-full border hover:scale-105 focus:scale-95 duration-300 border-neutral-300 px-6 py-3 text-sm font-semibold text-neutral-700 transition hover:border-neutral-400 hover:bg-neutral-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-white/20 dark:text-neutral-200 dark:hover:bg-white/10 flex items-center justify-center gap-2"
